...
phase_validate() {
  ...
  if command -v ollama &>/dev/null; then
    ...
    warn "Ollama not found. Auto-installing via Homebrew..."
    # Rely on HOMEBREW to install without user interaction
    HOMEBREW_NO_ENV_HINTS=1 brew install ollama </dev/null || die "brew install ollama failed."
    if command -v ollama &>/dev/null; then
      OLLAMA_BIN="$(command -v ollama)"
    elif [[ -x "${BREW_PREFIX}/bin/ollama" ]]; then
      OLLAMA_BIN="${BREW_PREFIX}/bin/ollama"
    else
      die "Auto-install finished but ollama binary still not found."
    fi
  fi
...
phase_configure() {
  ...
  # ── Tailscale auth key ────────────────────────────────────
  # Check if Tailscale is already authenticated on this machine.
  # If so, skip the auth key prompt entirely (unless --force).
  local _ts_already_authed=false
  if tailscale ip -4 &>/dev/null && tailscale ip -4 2>/dev/null | grep -qE '^100\.'; then
    _ts_already_authed=true
  fi

  if [[ "$_ts_already_authed" == "true" && "$FORCE_MODE" == "false" ]]; then
    local _existing_ts_ip
    _existing_ts_ip=$(tailscale ip -4 2>/dev/null || echo "")
    ok "Tailscale already authenticated (IP: ${_existing_ts_ip}) — skipping auth key prompt."
    ok "Use --force to re-authenticate with a new key."
    # Set a sentinel so step_setup_tailscale knows to skip re-auth
    TAILSCALE_AUTHKEY="__EXISTING__"
  else
...
step_harden_security() {
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    ...
    # Note: Explicit macOS ACLs like 'everyone deny' are avoided here
    # because they can inadvertently block the actual owner from writing.
    # POSIX 700 combined with the sandbox profile provides sufficient security.
  fi

  manifest_add "permission" "$OLLAMA_HOME"
  ok "Model dir secured: ${OLLAMA_HOME} (mode 700 + sandbox)"
...
;; ── Process ────────────────────────────────────────────────────
;; Allow execution of Ollama binary (required for running the model server)
;; Using process-exec (not file-exec*) - sandbox-exec uses this to allow binary execution
(allow process-exec (literal "${OLLAMA_BIN}"))
;; Also allow the actual binary behind the Homebrew symlink
(allow process-exec (subpath "${BREW_PREFIX}/Cellar/ollama"))
(allow process-exec (subpath "${BREW_PREFIX}/Cellar"))
...
;; ── /usr/local for direct-installed Ollama (not Homebrew) ───
...
(allow network-bind   (local ip "localhost:${OLLAMA_PORT}"))
(allow network-inbound (local ip "localhost:${OLLAMA_PORT}"))
...
PLIST
  ...
  # Strategy:
  #  0. If already authenticated (sentinel from phase_configure): skip re-auth,
  #     just ensure settings are applied (hostname, SSH, accept-dns).
  ...
  # Check if Tailscale is already authenticated and we should preserve the session
  if [[ "${TAILSCALE_AUTHKEY:-}" == "__EXISTING__" ]]; then
